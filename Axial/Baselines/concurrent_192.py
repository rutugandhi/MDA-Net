...
mc = ModelCheckpoint("concurrent_192_model_checkpoint.h5", monitor = 'val_loss', period = 100)
# parallel_model = multi_gpu_model(model, gpus = 3)

time_callback = TimeHistory()
model.compile(optimizer = Adam(lr = 5e-5), loss =  {'conv22':dice_coef_loss, 'conv23': dice_coef_loss, 'conv24': dice_coef_loss}, metrics = [dice_coef] )
history = model.fit([X_train], [y_train[:, :,:,0],y_train[ :,:,:,1],y_train[ :,:,:,2]], callbacks = [time_callback], batch_size=3, epochs=1, verbose = 2, shuffle=True)#, validation_data = (X_test[0:256], (y_test[0:256, :, :, 0], y_test[0:256, :, :, 1], y_test[0:256, :, :, 2] )) )
model.save("concurrent_unet_192_300_fold2.h5")
